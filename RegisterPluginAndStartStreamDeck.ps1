...
$pluginProjectFile = "$basePath\BluetoothController.csproj"
...
If (-not (Test-Path $bindir)) {
  Write-Error "The output directory `"$bindir`" was not found.`n You must first build the `"BluetoothController`" project before calling this script.";
  exit 1;
}
